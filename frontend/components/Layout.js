...
function DashboardContent({ children }) {
  ...
  const [newCageModal, setNewCageModal] = React.useState(false);
  const [updateCageModal, setUpdateCageModal] = React.useState(false);
  const [animalModal, setAnimalModal] = React.useState(false);

  const toggleNewCageModal = () => {
    setNewCageModal(!newCageModal);
    console.log(`modal state changed`);
  };

  const toggleUpdateCageModal = () => {
    setUpdateCageModal(!updateCageModal);
    console.log(`modal state changed`);
  };
      ...
      <Box sx={{ display: "flex" }}>
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            <Box>
              {newCageModal && <NewCage sx={{ zIndex: 100 }}></NewCage>}
              {updateCageModal && (
                <UpdateCage sx={{ zIndex: 100 }}></UpdateCage>
              )}
              {animalModal && (
                <NewAnimal
                  sx={{ zIndex: 100 }}
